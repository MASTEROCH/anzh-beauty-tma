...
/* Скидка за задания — решение владелицы (Роч, 24.09.2026):
     5% за отзыв · 5% за отметку в сторис · за оба условия 10%.

   ⚠️ РАСХОЖДЕНИЕ С КОНТРАКТОМ §4.7, отмечено сознательно. Там записано
   «10% за задание, до 30%» и приглашение подруги в числе заданий.
   Владелица правило изменила: приглашений в скидке больше нет, они
   остаются реферальной программой со своей механикой (§4.1–4.6 в силе,
   привязка пишется по-прежнему). Чем награждать приглашение — вопрос
   открыт, ответ за владелицей.

   Потолок ВЫВОДИТСЯ из числа условий, а не задаётся отдельно: число,
   которого нельзя достичь, хуже меньшего числа. */
export const PERCENT_PER_QUEST = 5;
export const QUEST_KINDS = ['review', 'story'] as const;
export const MAX_DISCOUNT_PERCENT = PERCENT_PER_QUEST * QUEST_KINDS.length;
